...
根据给定中心线曲线的控制点，对腔体模型沿各截面切割，
直接按 12.2 节规定输出 contour CSV（无需额外 VTK 文件）。

本版本 **不依赖** 额外的 2 个端点 Fiducial，仅使用曲线节点本身的控制点。
如欲忽略曲线首尾两个点，可将 `use_curve_endpoints` 设为 `False`。
...
使用方法：在 Slicer Python Console 执行：
    >>> exec(open('/path/to/slicer_extract_contour_csv_no_endpoints.py').read())
"""
...
# ============================== 可调参数 =====================================
curveName    = 'Centerline curve (0)'   # 曲线节点名称
modelName    = 'MyBox'                  # 腔体模型（封闭 lumen）
outputDir    = r'/tmp'                  # 输出目录，根据需要修改
csvName      = 'contour_auto_noEP.csv'  # 输出 CSV 文件名
...
num_pts = curve_node.GetNumberOfControlPoints()
if num_pts < 3:
    raise RuntimeError('曲线节点控制点数量不足 3，无法计算切向量')
...
cl_path = os.path.join(outputDir, 'centerline_noEP.csv')
